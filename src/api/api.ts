...
export default class Api {
    ...
    public async request<RequestType, ResponseType>(config: AxiosRequestConfig & { skipAuth?: boolean }) {
        const headers: RawAxiosRequestHeaders = {
            ...(config.headers?.["Content-Type"] != null ? { ...config.headers } : { "Content-Type": "application/json" }),
            ...(config.skipAuth
                ? {}
        ...
        console.log("Request URL:", path);  // Verifica la URL completa que se está enviando
